...
def train(args):
    ...
    text_encoder, unet, network = train_util.transform_if_model_is_DDP(
        text_encoder, unet, network
    )
    ...
    if accelerator.is_main_process:
        ...
        train_images = []
        reg_images = []
        for key, info in train_dataset_group.image_data.items():
            if not info.is_reg:
                wmg = wandb.Image(
                    open_and_thumbnail(info.absolute_path),
                    caption=f"{key}: {info.caption} x {info.num_repeats}",
                )
                train_images.append(wmg)
            elif info.is_reg and len(reg_images) < 50:
                wmg = wandb.Image(
                    open_and_thumbnail(info.absolute_path),
                    caption=f"{key}: {info.caption} x {info.num_repeats}",
                )
                reg_images.append(wmg)

        tracker.log(
            {
                "train images": train_images,
                "reg images": reg_images,
            }
        )
